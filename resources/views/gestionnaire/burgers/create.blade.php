...
<div class="max-w-xl mx-auto">
    <div class="flex items-center gap-3 mb-6">
        ...
        <h1 class="text-2xl font-bold text-gray-800">Nouveau burger</h1>
    </div>

    <div class="bg-white rounded-2xl shadow-sm border p-6"> 
        <form method="POST" action="{{ route('gestionnaire.burgers.store') }}" enctype="multipart/form-data" class="space-y-5">
            @csrf
            ...
            <div>
                ...
                <textarea name="description" rows="3"
                          class="w-full border rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-orange-400">{{ old('description') }}</textarea>
            </div>

            <div class="grid grid-cols-2 gap-4">
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Prix (FCFA) *</label>
                    <input type="number" name="prix" value="{{ old('prix') }}" min="0" required
                           class="w-full border rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-orange-400">
                </div>
                <div>
